...
const user = (function() { // создаем объектный интерфейс, модуль записываем в переменную и пытаемся в неё возвратить методы, доступные снаружи 
	...
	return { // применим объектный интерфейс, т.е. вернем (экспортируем) этой локальной анонимной функции объект через метод sayHallo, в который мы передаем ссылку на приватную функцию privat
		sayHallo: privat
	}; 
}());
