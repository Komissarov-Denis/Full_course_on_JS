...
const req = new Promise(function(resolve, reject) { // с помощью конструктора new Promise() создаем запрос и в него помещаем коллбэк функцию с аргументами resolve = выполнено и reject = не выполнено, эти аргументы обозначают тоже функции, которые можно передавать 
	setTimeout(() => { // т.е. направляем запрос на сервер и ждем получение результата product, получаем его через 2 сек.
		...
		resolve(product); // если успешно отработал асинхронный код product, запускается функция resolve(), в нее мы передаем данные объекта product{}
	}, 2000);
});
req.then((product) => { // метод then() будет выполняться на промисе в случае положительного результата обработки коллбэк функции и он принимает внутрь себя этот аргумент resolve(product) 
...
}); // ПРЕИУЩЕСТВО ПЕРЕД КОЛЛБЭКАМИ - МЫ МОЖЕМ ВОЗВРАЩАТЬ ПРОМИСЫ ПО ЦЕПОЧКЕ, ВЫПОЛНЕНИЕ КАЖДОЙ АСИНХРОВННОЙ ОПЕРАЦИИ ЗАПУСКАЕТ СЛЕДУЩУЮ!!!
// Получил с интервалом:
// Запрос данных.... 
// 2 сек Подготовка данных...
// 4 сек { name: 'TV', price: 2000, status: 'ordered' }
...
req.then((product) => { // для обработки положительного результата функции resolve() используем метод then(), он принимает внутрь себя этот аргумент из функции resolve()
