...
function showFirstMessage() { // имя функции должно начинаться с глагола действия функции
	console.log('Hello World!'); // получил: Hello World!, поименованная функция без аргумента!!!
}
showFirstMessage(); // обязательно необходимо вызывать функцию!!! пишем в формате КэмэллКейс!!!

function showFirstMessage1(text) { // поименованная функция с аргументом!!!
	console.log(text); // получил: Hello World!, аргументы указываются через запятые
...
showFirstMessage1('Hello World!'); // получил: Hello World!, text - аргумент функции
...
// FUNCTION EXPRESSION - ФУНКЦИОНАЛЬНОЕ ВЫРАЖЕНИЕ - ФУНКЦИЯ ПОМЕЩАЕТСЯ В ПЕРЕМЕННУЮ!!!
// ОНА ЗАПУСКАЕТСЯ ИМЕННО ТОГДА, КОГДА ОБЪЯВЛЯЕТСЯ ПЕРЕМЕННАЯ!!!
const logger = function () {
	console.log('Hello World!'); // получил: Hello World!
...
const amountRub = 1000; // Задача 1000 рублей обменять по курсу и рассчитать скидку на товар по курсу
...
function test() {
	for (let i = 0; i < 5; i++) {
		console.log(i);
		if (i === 3) return; // как только цикл в функции натыкается на строгое равенство 3, функция останавливается и DONE не выведется!!! 
	}
	console.log('DONE');
}
test();
