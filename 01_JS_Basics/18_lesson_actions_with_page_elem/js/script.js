...
//   hearts[i].style.backgroundColor = 'blue'; // но они практически не используются, взамен есть методы!!! =>
// }
...
const text = document.createTextNode('Тут был я!'); // создание текстового узла в JS файле, используется крайне редко
console.log(text);
...
divYellow.style.height = `${height}px`;
...
divMod.style.height = `${height + 30}px`;
...
carouselWrapper.classList.add('wrapper_crsl');
document.body.prepend(carouselWrapper);
carouselWrapper.style.cssText = 'display: flex; justify-content: center; align-items: center; margin: 0 auto; width: 1200px; height: 200px';
...
function replaced() {
	setTimeout(() => { 	
		for (let i = 0; i < 3; i++) { 
			setTimeout(() => { 
				carousel[i].after(replacer[i]);
				// carousel[i].after(replacer[i]);
				// carousel[i].replaceWith(replacer[i]); 
