...
// REF нельзя назначать на функциональные компоненты так как они не не являются классами и они не создают экземпляры, соответственно мы компонент преобразуем в классовый и тогда
// ошибки не будет!!!
...
class Form extends Component {
    ...
    } // используем метод ficusFistTextInput() на элементе <textarea/> по средством обработчика onClick, тогда при фокусе на второй input - идет перенаправление на первый input
