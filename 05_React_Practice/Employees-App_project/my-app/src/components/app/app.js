...
class App extends Component { // APP.js - данный компонент является источником истины, так как обязан контролировать все остальные компоненты и их данные 
	...
	deleteItem = (id) => { // таким образом через изменение состояния setState по иерархии можно передавать функцию по удалению
		this.setState(({data}) => { // вытаскиваем значение data из массива с объектами по уникальному идентификатору
			...
			//----первый способ-!!!!!!!!!!!!!!!-----
			// const index = data.findIndex(elem => elem.id === id); // c помощью метода findIndex() находим индекс объекта, с помощью коллбэк функции, которая при возврате true (т.е. найденный id элемента совпадает c id элемента-событие по клику)
			// console.log(index) // затем берем index и передаем в метод slice() для удаления нужного объекта внутри массива, все это будет изменять State для формирования нового состояния массива/объекта
			...
			// const after = data.slice(index + 1); // slice() - метод, копирующий часть массива и создающий новый с найденного индекса + 1 до конца, не включительно найденного индекса
			// const newArr = [...before, ...after];
			// return{ // далее возвращаем новый сформированный массив
			// 	data: newArr
			// }
			//--------------------------------------	
			...
			} // т.е. подходит для сохранения ИММУТАБЕЛЬНОСТИ ранее созданного объекта => data.filter() отфильтрует данные, в результате останутся только те элементы, ID которых не совпадают с ID удаляемого элемента-событие по клику  
			//--------------------------------------	
    ...
    addItem = (name, salary) => { // создаем новый объект с передачей ему аргументов /name, salary/
        ...
        this.setState(({data}) => { // передаем коллбэком данные через состояние нового объекта компонента 
            ...
            return {
                data: newArr // как итог: вернется новый массив
            }
        });
    }
	...
	//--------------------------------------------------------------------------------------------------------------------------------------------
	//--------------------------------------------------------------------------------------------------------------------------------------------	
	// onToggleIncrease = (id) => { // данный метод будет изменять значение increase (премия) на противоположный у определенного элемента по id			
		...
		// this.setState(({data}) => { // обращаемся к setState и вытаскиваем данные data
			...
			// const oldItem = data[index]; // oldItem - это старый объект, на котором произошло событие по клику /промежуточная переменная/ и получаем данные по индексу
			// const newItem = {...oldItem, increase: !oldItem.increase}; // методом разворота spread() разворачиваем в новом объекте сначала старый объект, что не нарушает принципов иммутабельности и мы можем добавлять новые свойства в него, при этом все что будет после запятой /{...oldItem, increase: !oldItem.increase}/ - будет заменять предыдущие данные на противоположные в новом объекте increase: false => на true!!!
			...
			// return { // т.е. первый аргумент разворачивает весь массив объектов до индекса-объекта-событие-клик, потом сам массив по индексу-объекта-событие-клик с измененным increase, затем весь массив после индекса-объекта-событие-клик  
	...
	// onToggleRise = (id) => { // данный метод будет изменять значение Rise (повышение должности) на противоположный у определенного элемента по id
	// 	this.setState(({data}) => ({ // возвращаем сразу объект данных со свойством data с помощью метода перебора массива map(), так как менять напрямую менять объект не можем
	// 		data: data.map(item => {  // item - это каждый объект внутри нашего нового массива data, соответственно: к перебору всех объектов массива /data/ коллбэк функцией, мы можем применить условие: 
	// 			if (item.id === id) { // если каждый ID объекта массива совпадает с тем ID, который к нам пришел внутри метода /ID найденного объекта, который мы хотим изменить прямо сейчас/
