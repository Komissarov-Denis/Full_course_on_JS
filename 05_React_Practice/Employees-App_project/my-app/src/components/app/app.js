...
class App extends Component { // APP.js - данный компонент является источником истины, так как обязан контролировать все остальные компоненты и их данные 
	...
	// onToggleIncrease = (id) => { // данный метод будет изменять значение increase (премия) на противоположный у определенного элемента по id			
		...
		// this.setState(({data}) => { // обращаемся к setState и вытаскиваем данные data
			...
			// const oldItem = data[index]; // oldItem - это старый объект /промежуточная переменная/ и получаем данные по индексу
			// const newItem = {...oldItem, increase: !oldItem.increase}; // методом разворота spread() разворачиваем в новом объекте старый объект, что не нарушает принципов иммутабельности и мы можем добавлять новые свойства в него, при этом все что будет после запятой /{...oldItem, increase: !oldItem.increase}/ - будет заменять предыдущие данные на противоположные в новом объекте increase: false => на true!!!
