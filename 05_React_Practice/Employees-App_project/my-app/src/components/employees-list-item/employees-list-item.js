...
// данный компонент уже не нужен, так как ранее предполагалось отслеживать их выполнение на локальном уровне, но теперь есть глобальные onToggleRise и onToggleIncrease, меняем компонент на стрелочную функцию с пропсами
	...
	// 	this.setState(({increase}) => ({ // это принцип деструктуризации с коллбэком, так как последующее состояние зависит от предыдущего this.state = {increase: false}, this.setState принимает в себя коллбэк с аргументом increase: false
	// 		increase: !increase // данным способом просто меняем значение ключевого свойства объекта increase на обратное (false на true) при нажатии на печеньку/кнопку
	// 	}))
