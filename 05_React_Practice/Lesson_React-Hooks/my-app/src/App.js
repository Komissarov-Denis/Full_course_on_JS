...
const Slider = (props) => { // компонент Slider с большой буквы прописываем и вводим состояние в функциональный компонент
	...
	const getSomeImages = useCallback(() => { // создадим функцию getSomeImages() и обернем в Хук useCallback(), с добавлением параметра для отслеживания [slide]
...
const Slide = ({getSomeImages}) => { // создаем функцию Slide(), который как Props /один из аргументов/ получает функцию getSomeImages(). Внутри этого функционального компонента можно создать новое
	const [images, setImages] = useState([]); // состояние useState([]) c массивом данных [images, setImages], и состояние с пустым массивом потом заполнится изображениями
	useEffect(() => { // когда компонент будет создан и если он, вдруг, начнет обновляться, то в таком случае будем менять количество изображений внутри слайдера. Применим Хук useEffect() и в него
		setImages(getSomeImages()) // поместим коллбэк функцию, которая запускается каждый раз либо при создании компонента, либо при его обновлении. Запустим функцию setImages(), которая устанавливает
	}, [getSomeImages]) // состояние в images, при этом как аргумент функцию getSomeImages() /запрос на сервер, на получение картинок или данных/ обернем в setImages(). Т.е. правильное поведение - это
	// запрос данных внутри useEffect(). Далее функция setImages() установит массив с картинками, но при этом необходимо прописать массив зависимостей от функции getSomeImages(), т.е. это означает, что
	// когда эта функция изменится, только в данном случае будет запускаться useEffect() повторно, а измениться она естественно может, когда мы передадим ее внутрь <Slider/>.
	return ( // далее возвращаем их этого компонента React фрагмент
		<>
			{images.map((url, i) => 
				<img 
					key={i}
