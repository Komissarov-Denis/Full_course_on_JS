...
class WhoAmI extends Component {
	...
	nextYear() { // метод, который мы передаем во внутрь класса, в данном случае работает не стрелочная функция в связи с (bind)!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		console.log('+++');
		this.setState(state => ({  
...
//-------------------------------------------------------------------------------------------------------
// 2. способ: (экспериментальный способ) применение синтаксиса полей классов и стрелочных функций
...
// по onClick запустится анонимная стрелочная функция: () => this.nextYear(), внутри будет запущен метод this.nextYear(), затем this укажет новый экземпляр класса
