// ПЕРВЫЙ СПОСОБ-------------------------------------------------------------------------------------------------------------------------------------
import { Component, Fragment } from 'react'; // ДОБАВЛЯЕМ Fragment
import './App.css';
...
class WhoAmI extends Component {
	...
	render() {
		...
		return ( // при запуске изменения setState() мы меняем только годы при нажатии на кнопку, поэтому в {this.state.text} ничего не поменяется !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			// ПРИМЕР применения ФРАГМЕНТА!!! скрывает не нужный <DIV>!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! НЕОБХОДИМ ЕСЛИ ЕСТЬ СТИЛИ FLEX, WIDTH: 100%, GRID  И Т.Д., ЧТОБЫ НЕ СЪЕХАЛА ВЕРСТКА
			<Fragment>
...
// ВТОРОЙ СПОСОБ--------------------------------------------------------------------------------------------------------
import { Component } from 'react'; // БЕЗ ИМПОРТА фрагмента, ЭТО ПРОИСХОДИТ АВТОМАТИЧЕСКИ
import './App.css';
